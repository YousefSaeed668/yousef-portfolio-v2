...
export function BlogCard({ post }: { post: BlogCardPost }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col rounded-2xl overflow-hidden bg-surface-dark border border-white/5 hover:border-primary/30 transition-all duration-500"
    >
      {post.coverImage ? (
        ...
        <div
          className={`h-24 w-full bg-linear-to-br ${gradient} transition-all duration-500 group-hover:h-28`}
        />
      )}
